\begin{figure}
\centering
\includegraphics{assets/fpt_logo.png}
\caption{FPT University Logo}
\end{figure}

\subsection{CAPSTONE PROJECT REPORT}\label{capstone-project-report}

\subsection{DEEP LEARNING FOR ANOMALY DETECTION IN INDUSTRIAL IMAGES}\label{deep-learning-for-anomaly-detection-in-industrial-images}

\textbf{Project Code:} SP26AI55\\
\textbf{Group Code:} GSP26AI27

\textbf{Research Team:} \textbar{} Full Name \textbar{} Student ID \textbar{} \textbar:---\textbar:---\textbar{} \textbar{} NGUYEN HOANG MINH SON \textbar{} SE151025 \textbar{} \textbar{} ON NGUYEN THIEN PHUC \textbar{} SE172629 \textbar{} \textbar{} NGUYEN DANG THAI BINH \textbar{} SE183718 \textbar{} \textbar{} LE THANH THAO NHI \textbar{} SE172759 \textbar{}
...
\item
  \textbf{CHAPTER 2: RELATED WORK}
\item
  \textbf{CHAPTER 3: FEATURE BACKBONE}
\item
  \textbf{CHAPTER 4: THE PATCHCORE ENGINE}
\item
  \textbf{CHAPTER 5: OPTIMIZATION INTERNALS}
\item
  \textbf{CHAPTER 6: SYSTEM PIPELINE \& CALIBRATION}
\item
  \textbf{CHAPTER 7: AGENTIC ML \& EXPLAINABLE AI}
\item
  \textbf{CHAPTER 8: EXPERIMENTS \& EVALUATION}
\item
  \textbf{CHAPTER 9: SYSTEM DEMONSTRATION}
\item
  \textbf{CHAPTER 10: PROJECT MANAGEMENT PLAN}
\item
  \textbf{CHAPTER 11: CONCLUSION \& FUTURE WORK}
\item
  \textbf{REFERENCES}
\item
  \textbf{APPENDICES}
...
\begin{quote}
\textbf{How can we maintain PatchCore's near-perfect anomaly detection accuracy while reducing its inference-time computational cost to levels suitable for real-time deployment on resource-constrained industrial hardware (≤ 6GB VRAM)?}
\end{quote}
...
\section{CHAPTER 2: RELATED WORK --- A Critical Analysis of Unsupervised Anomaly Detection Paradigms}\label{chapter-2-related-work-a-critical-analysis-of-unsupervised-anomaly-detection-paradigms}
...
\section{CHAPTER 3: FEATURE BACKBONE --- Hierarchical Representation Learning via ResNet-18}\label{chapter-3-feature-backbone-hierarchical-representation-learning-via-resnet-18}
...
\subsubsection{3.1.1 Spatial Normalization: The 256 × 256 Contract}\label{spatial-normalization-the-256-256-contract}
...
\subsubsection{3.2.1 Conv1: The 7×7 Convolution}\label{conv1-the-77-convolution}
...
\bottomrule\noalign{}
\endlastfoot
Layer 1 & 64 & 64×64 & \textasciitilde11px & Edges, gradients & Too primitive \\
\textbf{Layer 2} & \textbf{128} & \textbf{32×32} & \textbf{\textasciitilde43px} & \textbf{Textures, patterns} & \textbf{High (local defects)} \\
\textbf{Layer 3} & \textbf{256} & \textbf{16×16} & \textbf{\textasciitilde99px} & \textbf{Part structures} & \textbf{High (context)} \\
Layer 4 & 512 & 8×8 & \textasciitilde211px & Object-level semantics & Too abstract \\
\end{longtable}
...
\textbf{Why Layer 2 + Layer 3?} The concatenation of Layer 2 (128-d, texture-sensitive, 32×32 resolution) and Layer 3 (256-d, structure-sensitive, 16×16 resolution, upsampled to 32×32 via bilinear interpolation) produces a \textbf{384-dimensional multi-scale embedding} that simultaneously encodes: - \emph{What the local surface looks like} (from Layer 2) - \emph{Where in the global object structure this surface belongs} (from Layer 3)
...
\bottomrule\noalign{}
\endlastfoot
Stem (Conv1 + Pool) & (B, 64, 64, 64) & \textasciitilde154M \\
Layer 1 & (B, 64, 64, 64) & \textasciitilde302M \\
Layer 2 & (B, 128, 32, 32) & \textasciitilde302M \\
Layer 3 & (B, 256, 16, 16) & \textasciitilde302M \\
\textbf{Total (to Layer 3)} & --- & \textbf{\textasciitilde1,060M MACs} \\
\end{longtable}

Table 3.3: Computational cost analysis by backbone stage (MAC operations for 256x256 input).

On a modern GPU, \textasciitilde1,060M MACs translate to approximately \textbf{1.5ms inference time} --- effectively negligible compared to the memory bank search cost analyzed in Chapter 5. This confirms that the backbone is not the performance bottleneck; the search algorithm is.
...
\section{CHAPTER 4: THE PATCHCORE MEMORY ENGINE --- Coreset Construction and Anomaly Scoring}\label{chapter-4-the-patchcore-memory-engine-coreset-construction-and-anomaly-scoring}
...
\begin{itemize}
\item
  \textbf{Layer 2 only (128-d, 32×32)}: A texture patch from the cap region and the body region of a bottle may produce similar feature vectors despite belonging to entirely different structural contexts. The memory bank cannot distinguish position-dependent normality.
\item
...
Memory (MB)
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Search FLOPs (per image)
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Expected AUROC Impact
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
...
\section{CHAPTER 5: OPTIMIZATION INTERNALS --- Approximate Search and Hardware-Aware Distance Computation}\label{chapter-5-optimization-internals-approximate-search-and-hardware-aware-distance-computation}
...
On a mid-range GPU (e.g., GTX 1660 with \textasciitilde5 TFLOPS FP32), this translates to approximately \textbf{1.6ms} --- seemingly fast. However, this analysis ignores memory bandwidth. The 10,240 coreset vectors (each 384 × 4 bytes = 1,536 bytes) total 15.7 MB. For each of the 1,024 query patches, the entire coreset must be loaded from GPU global memory into registers. With a memory bandwidth of \textasciitilde192 GB/s:
...
Even after LSH reduces the candidate set to \textasciitilde33 vectors per query, the individual distance computations remain a bottleneck when accumulated over 1,024 query patches. The naive Euclidean distance:
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{dist\_sq }\OperatorTok{=}\NormalTok{ query\_norms }\OperatorTok{+}\NormalTok{ candidate\_norms }\OperatorTok{{-}} \DecValTok{2} \OperatorTok{*}\NormalTok{ dot\_products}
\NormalTok{dist\_sq }\OperatorTok{=}\NormalTok{ torch.clamp(dist\_sq, }\BuiltInTok{min}\OperatorTok{=}\FloatTok{0.0}\NormalTok{)  }\CommentTok{\# Prevent NaN from sqrt of negative}
\NormalTok{distances }\OperatorTok{=}\NormalTok{ torch.sqrt(dist\_sq)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Precomputed during fit:}
\VariableTok{self}\NormalTok{.coreset\_norms }\OperatorTok{=}\NormalTok{ (}\VariableTok{self}\NormalTok{.memory\_bank }\OperatorTok{**} \DecValTok{2}\NormalTok{).}\BuiltInTok{sum}\NormalTok{(dim}\OperatorTok{=}\DecValTok{1}\NormalTok{)  }\CommentTok{\# Shape: (|M\_C|,)}

\CommentTok{\# At inference (per{-}bucket or batched):}
\NormalTok{query\_norms }\OperatorTok{=}\NormalTok{ (queries }\OperatorTok{**} \DecValTok{2}\NormalTok{).}\BuiltInTok{sum}\NormalTok{(dim}\OperatorTok{=}\DecValTok{1}\NormalTok{, keepdim}\OperatorTok{=}\VariableTok{True}\NormalTok{)    }\CommentTok{\# Shape: (|Q|, 1)}
\NormalTok{dot\_products }\OperatorTok{=}\NormalTok{ torch.mm(queries, candidates.T)            }\CommentTok{\# Shape: (|Q|, |B|)}
\NormalTok{distances }\OperatorTok{=}\NormalTok{ query\_norms }\OperatorTok{+}\NormalTok{ candidate\_norms }\OperatorTok{{-}} \DecValTok{2} \OperatorTok{*}\NormalTok{ dot\_products  }\CommentTok{\# Broadcasting}
\NormalTok{distances }\OperatorTok{=}\NormalTok{ torch.clamp(distances, }\BuiltInTok{min}\OperatorTok{=}\FloatTok{0.0}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\bottomrule\noalign{}
\endlastfoot
Candidate selection & All 10,240 & \textasciitilde33 per query & 310× \\
Distance computation & Element-wise loop & Matrix multiplication & 3--5× \\
\textbf{Combined} & \textbf{Baseline} & \textbf{Both optimizations} & \textbf{\textasciitilde1,000--1,500×} (derived from Table 5.1; end-to-end latency in §8.6) \\
\end{longtable}
...
\subsubsection{5.5.1 LSH × Coreset Interaction}\label{lsh-coreset-interaction}
...
\subsubsection{5.5.2 LSH × Anomaly Scoring Interaction}\label{lsh-anomaly-scoring-interaction}
...
\section{CHAPTER 6: SYSTEM ARCHITECTURE \& DEPLOYMENT PIPELINE --- From Raw Images to Actionable Decisions}\label{chapter-6-system-architecture-deployment-pipeline-from-raw-images-to-actionable-decisions}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Double{-}check locking pattern for thread{-}safe model loading}
\ControlFlowTok{if}\NormalTok{ category }\KeywordTok{not} \KeywordTok{in} \VariableTok{self}\NormalTok{.\_loaded\_models:}
    \ControlFlowTok{with} \VariableTok{self}\NormalTok{.\_lock:}
        \ControlFlowTok{if}\NormalTok{ category }\KeywordTok{not} \KeywordTok{in} \VariableTok{self}\NormalTok{.\_loaded\_models:  }\CommentTok{\# Double{-}check}
            \VariableTok{self}\NormalTok{.\_loaded\_models[category] }\OperatorTok{=} \VariableTok{self}\NormalTok{.\_load\_model(category)}
\end{Highlighting}
\end{Shaded}
...
Feature maps (Layers 1--3) & \textasciitilde18.0 & Per-image \\
Coreset memory bank & 15.7 & Persistent \\
LSH index structures & \textasciitilde2.0 & Persistent \\
Distance computation workspace & \textasciitilde5.0 & Per-image \\
PyTorch CUDA overhead & \textasciitilde800.0 & Persistent \\
\textbf{Total (1 image)} & \textbf{\textasciitilde886.5} & --- \\
\textbf{Available for batch} & \textbf{\textasciitilde5,256} & --- \\
\end{longtable}

Table 6.3: VRAM memory budget breakdown for concurrent three-model deployment.

The available headroom of \textasciitilde5,256 MB for batch processing would theoretically allow up to 5 concurrent images. However, GPU memory fragmentation and non-deterministic CUDA allocator behavior reduce the practical safe limit.
...
\subsubsection{6.6.3 Cross-Module Interaction: Batching × LSH}\label{cross-module-interaction-batching-lsh}
...
\section{CHAPTER 7: AGENTIC ML \& EXPLAINABLE AI --- Closing the Optimization Loop}\label{chapter-7-agentic-ml-explainable-ai-closing-the-optimization-loop}
...
\emph{Proposal: Increase coreset\_ratio to 0.2 (retain more boundary patches) and increase k\_neighbors to 3 (smooth noise from texture variability). Expected impact: AUROC improvement of 5-10\% with \textasciitilde2× increase in inference time, which remains within acceptable limits.''}
\end{quote}
...
\tightlist
\item
  \textbf{Performance ceiling}: All categories achieve AUROC ≥ 0.95 (or a user-defined target).
\item
...
The anomaly detection pipeline produces three outputs per image: 1. A scalar \textbf{Anomaly Index} (e.g., \(I = 1.47\)) 2. A spatial \textbf{Anomaly Heatmap} (32×32, upsampled to input resolution) 3. A binary \textbf{Decision} (Normal / Anomalous)
...
\subsection{7.6 Cross-Module Interaction: Agentic Layer × Pipeline}\label{cross-module-interaction-agentic-layer-pipeline}
...
\bottomrule\noalign{}
\endlastfoot
Agent → Coreset (Ch. 4) & Adjusts \texttt{coreset\_ratio} & Changes memory bank size, affecting search cost and coverage \\
Agent → LSH (Ch. 5) & Could adjust \texttt{lsh\_bits} & Changes bucket count, affecting search recall \\
...
\textbf{The constraint propagation problem.} When the agent proposes increasing \texttt{coreset\_ratio} from 0.1 to 0.2, this doubles the memory bank size, which: 1. Doubles the LSH search candidate count per bucket 2. Increases VRAM usage by \textasciitilde12 MB 3. Requires recalibrating the threshold (the score distribution shifts with coreset size)
...
\section{CHAPTER 8: EXPERIMENTS \& EVALUATION --- Empirical Validation on MVTec AD}\label{chapter-8-experiments-evaluation-empirical-validation-on-mvtec-ad}
...
\bottomrule\noalign{}
\endlastfoot
Total training images (normal only) & \textasciitilde3,629 \\
Total test images (normal + anomalous) & \textasciitilde1,725 \\
Normal test images & \textasciitilde467 \\
Anomalous test images & \textasciitilde1,258 \\
...
RAM & 16 GB DDR4 \\
Storage & NVMe SSD (read: \textasciitilde3 GB/s) \\
...
\textbf{V2} & + Coreset subsampling (10\%) & Functional & \textasciitilde4.5s & \textasciitilde3.2 GB \\
\textbf{V3} & + LSH indexing (12-bit) & Faster search & \textasciitilde1.2s & \textasciitilde3.3 GB \\
\textbf{V4} & + XOR-Probe + Matrix distance & Sub-second & \textasciitilde0.5s & \textasciitilde3.3 GB \\
\textbf{V5} & + Knowledge Distillation + CLAHE & Production-ready & \textbf{\textasciitilde0.4s} & \textbf{\textasciitilde3.4 GB} \\
\end{longtable}
...
\bottomrule\noalign{}
\endlastfoot
SPADE & 2020 & WideResNet-50 & 0.855 & \textasciitilde200ms & 8GB+ \\
PaDiM & 2021 & WideResNet-50 & 0.975 & \textasciitilde80ms & 4GB \\
FastFlow & 2022 & WideResNet-50 & 0.985 & \textasciitilde50ms & 10GB+ \\
PatchCore (Vanilla) & 2022 & WideResNet-50, 100\% & \textbf{0.991} & \textgreater10s (OOM) & \textgreater12GB \\
CFlow-AD & 2022 & WideResNet-50 & 0.987 & \textasciitilde100ms & 8GB+ \\
\textbf{Ours (PatchCore-Lite)} & \textbf{2026} & \textbf{ResNet-18, 0.5--18\%} & \textbf{0.736} & \textbf{\textasciitilde400ms} & \textbf{\textasciitilde3.4GB} \\
\textbf{Ours (Best Model)} & \textbf{2026} & \textbf{CNN-OCSVM ensemble} & \textbf{0.787} & \textbf{\textasciitilde200ms} & \textbf{\textasciitilde2.8GB} \\
\end{longtable}
...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Backbone asymmetry.} Vanilla PatchCore uses WideResNet-50 (68.9M parameters, 1,792-D features), while our system uses ResNet-18 (11.7M parameters, 384-D features). This is a \textbf{6× parameter reduction} that directly impacts feature discriminability.
\item
...
\bottomrule\noalign{}
\endlastfoot
Exact search (no LSH) & 0.980 & \textasciitilde400 & 100\% \\
LSH 8-bit, no XOR & 0.961 & \textasciitilde80 & \textasciitilde72\% \\
LSH 12-bit, no XOR & 0.948 & \textasciitilde60 & \textasciitilde68\% \\
\textbf{LSH 12-bit + XOR-Probe} & \textbf{0.978} & \textbf{\textasciitilde100} & \textbf{\textasciitilde95\%} \\
LSH 16-bit + XOR-Probe & 0.979 & \textasciitilde120 & \textasciitilde96\% \\
\end{longtable}
...
\bottomrule\noalign{}
\endlastfoot
ResNet-18 (frozen, ImageNet) & 0.724 & \textasciitilde1.5 & 11.7M & ✅ \\
\textbf{CustomResNet18 (KD-trained)} & \textbf{0.736} & \textbf{\textasciitilde1.2} & \textbf{\textasciitilde10M} & \textbf{✅} \\
ResNet-34 (frozen, ImageNet) & \textasciitilde0.78* & \textasciitilde2.8 & 21.8M & ⚠️ Marginal \\
WideResNet-50 (frozen) & 0.991† & \textasciitilde8.5 & 68.9M & ❌ OOM \\
\end{longtable}
...
\textbf{256 (projected)} & \textbf{\textasciitilde0.732} & \textbf{0.67×} & \textbf{0.67× original} \\
\textbf{128 (projected)} & \textbf{\textasciitilde0.720} & \textbf{0.33×} & \textbf{0.33× original} \\
64 (projected) & \textasciitilde0.685 & 0.17× & 0.17× original \\
\end{longtable}
...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Category-specific adaptation is essential.} The optimal coreset\_ratio varies 93× across categories (0.005 for Carpet/Bottle to 0.464 for Zipper), confirming that no single default configuration is adequate for diverse product types.
\item
  \textbf{The agent learned speed-accuracy trade-offs autonomously.} In 12 out of 15 categories, the agent converged to coreset\_ratio ≤ 0.02, discovering independently that aggressive coreset compression yields the best score when inference latency is penalized.
\item
...
\bottomrule\noalign{}
\endlastfoot
Image loading + CLAHE & \textasciitilde15 & 3.8\% \\
Resize + Normalize & \textasciitilde2 & 0.5\% \\
Backbone forward pass & \textasciitilde1.2 & 0.3\% \\
Feature extraction + pooling & \textasciitilde3 & 0.8\% \\
LSH hashing (12 dot products) & \textasciitilde0.5 & 0.1\% \\
XOR-Probe bucket lookup & \textasciitilde1.0 & 0.3\% \\
Distance computation (matrix) & \textasciitilde8 & 2.0\% \\
Score aggregation + Anomaly Index & \textasciitilde0.3 & 0.1\% \\
Heatmap generation + upsampling & \textasciitilde20 & 5.0\% \\
\textbf{Result serialization + I/O} & \textbf{\textasciitilde350} & \textbf{87.5\%} \\
\textbf{Total} & \textbf{\textasciitilde400} & \textbf{100\%} \\
\end{longtable}

Table 8.9: Latency profiling breakdown for single-image inference.

\textbf{Critical finding.} The neural network computation (backbone + search + scoring) accounts for only \textasciitilde14 ms (\textless4\%) of the total pipeline latency. The dominant cost is \textbf{I/O and visualization}: saving anomaly heatmaps, writing JSON results, and generating report visualizations. This reveals that further optimization of the ML pipeline yields diminishing returns --- the next performance frontier is I/O optimization (async file writes, memory-mapped output buffers).
...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{High intra-class variability + ultra-aggressive coreset}: Both categories use 0.5\% coreset ratio --- the most aggressive setting. Carpet has high texture variability (color gradients, pile direction), and Cable has complex multi-component geometry. A 0.5\% coreset retains approximately 51 patch embeddings from \textasciitilde10,240 --- far too few to represent the full range of normal appearances.
\item
...
\section{CHAPTER 9: SYSTEM DEMONSTRATION \& DEPLOYMENT}\label{chapter-9-system-demonstration-deployment}
...
\section{CHAPTER 10: PROJECT MANAGEMENT PLAN}\label{chapter-10-project-management-plan}
...
Member Name
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Core Academic \& Engineering Responsibilities
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
...
Duration
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Objective \& Key Output Deliverables
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
Status
\end{minipage} \\
\midrule\noalign{}
...
\section{CHAPTER 11: CONCLUSION \& FUTURE WORK}\label{chapter-11-conclusion-future-work}
...
\item
  \textbf{Single-scale detection.} The 32×32 feature map provides a fixed spatial resolution for anomaly detection. Defects smaller than the effective receptive field (\textasciitilde43 pixels) are systematically underdetected.
\end{enumerate}
...
\subsection{Appendix A: System Architecture Diagram}\label{appendix-a-system-architecture-diagram}
...
\subsection{Appendix B: Optimized Hyperparameter Table}\label{appendix-b-optimized-hyperparameter-table}
...
\subsection{Appendix C: Algorithmic Complexity Proof Sketch}\label{appendix-c-algorithmic-complexity-proof-sketch}
...
\subsection{Appendix D: Glossary of Key Terms}\label{appendix-d-glossary-of-key-terms}
